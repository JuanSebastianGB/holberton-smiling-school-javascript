...
const setTutorials = async () => {
  try {
    ...
    $('body').append(TutorialSection(fetchTutorials));
    $('body').append(Loader);
...
const setLatestVideos = async () => {
  try {
    const fetchLatestsVideos = await $.get(API_URL_TUTORIALS_LATEST);
    ...
    $('body').append(Loader);
    const tutorialListLength = fetchLatestsVideos.length;
    let middleLength = parseInt(tutorialListLength / 2);
    let counter = 0;
    fetchLatestsVideos.map((element) => {
      element.sub_title = element['sub-title'];
      if (counter < middleLength) {
        counter++;
        $('.row-active').append(TutorialCart(element));
